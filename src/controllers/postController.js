...
// Everyone can view feed (paginated basic)
...
exports.getComments = async (req, res) => {
  try {
    ...
    return res.json({ success: true, postId, count: comments.length, comments });
  } catch (error) {
    console.error(error);
...
exports.downloadPostImage = async (req, res) => {
  try {
    ...
    if (!url) return res.status(400).json({ success: false, message: "No image URL" });
    ...
    const filename =
      post.blobName ||
      `photo-${post._id}.${
        contentType.includes("png") ? "png" : contentType.includes("jpeg") ? "jpg" : "img"
      }`;
